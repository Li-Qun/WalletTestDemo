...
public class Wallet {
    public static let defaultPath = "m/44'/60'/0'/0/x"  //BIP44：基于 BIP32 的系统，赋予树状结构中的各层特殊的意义。让同一个 seed 可以支援多币种、多帐户等。m / purpose' / coin_type' / account' / change / address_index ，其中的 purporse' 固定是 44'，代表使用 BIP44。而 coin_type' 用来表示不同币种，例如 Bitcoin 就是 0'，Ethereum 是 60'。
