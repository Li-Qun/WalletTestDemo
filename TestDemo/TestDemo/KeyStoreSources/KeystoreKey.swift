...
public struct KeystoreKey {
    ...
    /// Mnemonic derivation path 助记词派生路径
    ...
    public init(password: String, type: AccountType) throws {
        ...
        case .hierarchicalDeterministicWallet:
            let mnemonic = Mnemonic.generate(strength: 256)//数据块256位
            try self.init(password: password, mnemonic: mnemonic, passphrase: "")
        }
    }

    /// Initializes a `Key` from a JSON wallet.
    public init(contentsOf1 json: Data) throws {
        self = try JSONDecoder().decode(KeystoreKey.self, from: json)
