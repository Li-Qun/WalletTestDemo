...
public final class KeyStore {
    ...
    public var keysByAddress = [Address: KeystoreKey]()
    ...
    private func load() throws {
        let accountURLs = FIRKeyChainManager.shareEngine().getAllKeys()
        for url in accountURLs! {
            do {
                let json = FIRKeyChainManager.shareEngine().getJsonData(withKey: url as! String)
                let key = try KeystoreKey(contentsOf1: json!)
                keysByAddress[key.address] = key
                let account = Account(address: key.address, type: key.type, url: url as! String)
    ...
    public func delete(account: Account, password: String) throws {
        ...
        //删除钱包
        FIRKeyChainManager.shareEngine().removeWalletObject(withKey: account.url)
    ...
    private func makeAccountURL(for address: Address) -> String {
    ...
    private func save(key: KeystoreKey, to url: String) throws {
        let json = try JSONEncoder().encode(key)
        //添加钱包
        FIRKeyChainManager.shareEngine().saveWalletObject(json, key: key.address.eip55String)
